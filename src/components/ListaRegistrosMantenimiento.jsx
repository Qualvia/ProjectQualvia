...
import { Loader2, Trash2, Filter, CalendarDays, User, ChevronDown, Mail } from "lucide-react";
import { format, startOfDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth } from "date-fns";
import { es } from "date-fns/locale";

const PAGE_SIZE = 30;
const EMPTY_FILTERS = { periodo: "todos", desde: "", hasta: "", equipo: "todos", tipo: "todos", estado: "todos" };
const ESTADO_STYLES = {
  "Operativo":           { badge: "bg-[#e6f5ed] text-[#2d8a5e]",    card: "bg-white border-border" },
  "No operativo":        { badge: "bg-red-50 text-red-500",           card: "bg-red-50 border-red-200" },
  "Pendiente revisión":  { badge: "bg-yellow-50 text-yellow-700",     card: "bg-yellow-50 border-yellow-200" },
};
    ...
    <div className="bg-secondary p-5 space-y-5 border-b border-border/40">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select value={local.estado} onChange={(e) => set("estado", e.target.value)}
            className="w-full h-9 rounded-lg border border-input bg-white px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
            <option value="todos">Todos</option>
...
function aplicarFiltros(registros, filtros) {
  return registros.filter((r) => {
    ...
    if (filtros.tipo !== "todos" && r.tipo_mantenimiento !== filtros.tipo) return false;
    if (filtros.estado !== "todos" && r.estado_final !== filtros.estado) return false;
    return true;
  });
}

export default function ListaRegistrosMantenimiento({ refreshKey }) {
  ...
  async function fetchPage(skipVal, replace = false) {
    ...
    const data = await base44.entities.RegistroMantenimiento.filter({ business_id: currentBusiness.id }, "-fecha", PAGE_SIZE + 1, skipVal);
  ...
  async function handleDelete(id) {
    await base44.entities.RegistroMantenimiento.delete(id);
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-border">
      ...
      <div className="p-4 space-y-3">
        ...
        {filtrados.map((r) => {
          const styles = ESTADO_STYLES[r.estado_final] || ESTADO_STYLES["Operativo"];
          return (
            <div key={r.id} className={`rounded-xl border px-5 py-4 ${styles.card}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-foreground text-base">{r.equipo_nombre}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${styles.badge}`}>{r.estado_final?.toLowerCase()}</span>
                  </div>
                  {r.tipo_mantenimiento && <p className="text-sm text-foreground mb-0.5"><span className="font-semibold">Tipo: </span><span className="text-[#6BB68A]">{r.tipo_mantenimiento}</span></p>}
                  {r.descripcion && <p className="text-sm text-foreground mb-0.5"><span className="font-semibold">Descripción: </span>{r.descripcion}</p>}
                  {r.proxima_revision && <p className="text-sm text-foreground mb-0.5"><span className="font-semibold">Próxima revisión: </span>{format(new Date(r.proxima_revision), "d MMM yyyy", { locale: es })}</p>}
                  {r.observaciones && <p className="text-xs text-muted-foreground italic mt-1">{r.observaciones}</p>}
                  {r.foto_url && <img src={r.foto_url} alt="evidencia" className="h-20 rounded-lg object-cover border border-border mt-2" />}
                ...
                <div className="flex gap-2 ml-3 shrink-0">
                  {r.foto_url && <a href={r.foto_url} target="_blank" rel="noreferrer" className="text-[#6BB68A] hover:opacity-70 transition-opacity"><Mail className="w-4 h-4" /></a>}
