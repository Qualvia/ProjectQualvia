  ...
  return (
    <div className="bg-secondary p-5 space-y-5 border-b border-border/40">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={handleAplicar} className="py-2.5 rounded-xl bg-[#6BB68A] hover:bg-[#5aa377] text-white font-semibold text-sm transition-colors">
          Aplicar filtros
        </button>
        <button onClick={handleLimpiar} className="py-2.5 rounded-xl bg-white border border-border text-foreground font-semibold text-sm hover:bg-secondary transition-colors">
          Limpiar filtros
        </button>
      </div>
    </div>
...
export default function ListaRegistrosMantenimiento({ refreshKey }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    base44.entities.RegistroMantenimiento.filter({ business_id: currentBusiness.id }, "-fecha", 200)
            ...
            <div key={r.id} className={`rounded-xl border px-5 py-4 ${styles.card}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {r.tipo_mantenimiento && (
                    <p className="text-sm text-foreground mb-0.5">
                      ...
                      <span className="text-[#6BB68A]">{r.tipo_mantenimiento}</span>
                    </p>
                  )}
                  {r.descripcion && (
                    <p className="text-sm text-foreground mb-0.5">
                  ...
                  {r.proxima_revision && (
                    <p className="text-sm text-foreground mb-0.5">
                      <span className="font-semibold">Próxima revisión: </span>
                      {format(new Date(r.proxima_revision), "d MMM yyyy", { locale: es })}
                    </p>
                  )}
                  {r.observaciones && (
                  ...
                  {r.foto_url && (
                    <img src={r.foto_url} alt="evidencia" className="h-20 rounded-lg object-cover border border-border mt-2" />
                  )}
                ...
                <div className="flex gap-2 ml-3 shrink-0">
                  {r.foto_url && (
                    <a href={r.foto_url} target="_blank" rel="noreferrer" className="text-[#6BB68A] hover:opacity-70 transition-opacity">
