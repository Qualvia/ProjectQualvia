              ...
              <div className="flex-1">
                ...
                {r.alergenos?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {r.alergenos.map((a) => (
                      <span key={a} className={`text-xs px-2.5 py-1 rounded-full border font-medium ${ALERGENO_COLORS[a] || "bg-yellow-100 text-yellow-700 border-yellow-300"}`}>
                        {a}
                      </span>
                    ))}
                  </div>
                ...
                {r.medidas_preventivas && (
                  <p className="text-sm text-foreground mb-0.5">
                    <span className="font-semibold">Medidas preventivas: </span>{r.medidas_preventivas}
                  </p>
                )}
                {r.observaciones && (
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                  ...
                  {r.origen === "recepcion" && (
                    <>
                      <span>·</span>
                      <span className="text-[#6BB68A] font-medium">Desde recepción</span>
                    </>
                  )}
                </div>
              </div>

              <button onClick={() => handleDelete(r.id)} className="text-destructive/60 hover:text-destructive transition-colors ml-3 shrink-0">
