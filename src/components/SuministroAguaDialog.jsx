...
import { Droplets, Upload, Loader2, FileText } from "lucide-react";
import { base44 } from "@/api/base44Client";
import { useBusiness } from "@/contexts/BusinessContext";

export default function SuministroAguaDialog({ open, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    base44.entities.SuministroAgua.filter({ business_id: currentBusiness.id })
      .then((data) => {
        if (data.length > 0) {
          setForm(data[0]);
          setExistingId(data[0].id);
        } else {
          setForm({ nombre_empresa: "", email: "", telefono: "", contacto: "", direccion: "", contrato_url: "" });
          setExistingId(null);
        }
      })
      .catch(() => {});
  ...
  async function handleSave() {
    ...
    if (existingId) {
      await base44.entities.SuministroAgua.update(existingId, payload);
    } else {
      await base44.entities.SuministroAgua.create(payload);
    }
    setSaving(false);
    onOpenChange(false);
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 pt-2">
          <div>
            ...
            <Input
              placeholder="Ej: Aguas de Valencia S.A."
          ...
          <div>
            ...
            {form.contrato_url ? (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-border bg-secondary">
                ...
                <a href={form.contrato_url} target="_blank" rel="noreferrer" className="text-sm text-[#6BB68A] underline flex-1 truncate">
                  Ver contrato subido
                </a>
                <button onClick={() => setForm((p) => ({ ...p, contrato_url: "" }))} className="text-xs text-muted-foreground hover:text-destructive">
                  Eliminar
                </button>
              </div>
            ) : (
