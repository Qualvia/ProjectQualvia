...
export default function ListaRegistrosPlagas({ refreshKey }) {
  ...
  async function fetchPage(skipVal, replace = false) {
    ...
    const data = await base44.entities.RegistroPlaga.filter({ business_id: currentBusiness.id }, "-fecha", PAGE_SIZE + 1, skipVal);
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-foreground text-base">{r.tipo_elemento ? `${r.tipo_elemento} - ${r.punto_nombre}` : r.punto_nombre}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${styles.badge}`}>{styles.label}</span>
                    {r.evidencia_plaga && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-red-100 text-red-600">⚠ Evidencia detectada</span>}
                  </div>
                  {r.ubicacion && <p className="text-sm text-muted-foreground mb-1">{r.ubicacion}</p>}
