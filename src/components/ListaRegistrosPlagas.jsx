...
const LIMITE = 5;
const EMPTY_FILTERS = { periodo: "todos", desde: "", hasta: "", punto: "todos", estado: "todos" };

const ESTADO_STYLES = {
  correcto:         { badge: "bg-[#e6f5ed] text-[#2d8a5e]",  card: "bg-white border-border",   label: "correcto" },
  incidencia:       { badge: "bg-red-50 text-red-500",         card: "bg-red-50 border-red-200", label: "incidencia" },
  requiere_revision:{ badge: "bg-yellow-50 text-yellow-700",   card: "bg-yellow-50 border-yellow-200", label: "requiere revisión" },
};
...
function FiltrosPanel({ registros, filtros, setFiltros, onClose }) {
  ...
  return (
    <div className="bg-secondary p-5 space-y-5 border-b border-border/40">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Punto de control</label>
...
export default function ListaRegistrosPlagas({ refreshKey }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    base44.entities.RegistroPlaga.filter({ business_id: currentBusiness.id }, "-fecha", 200)
      .then((data) => { setRegistros(data); setLoading(false); })
      .catch(() => { setRegistros([]); setLoading(false); });
  }, [currentBusiness, refreshKey]);

  async function handleDelete(id) {
    await base44.entities.RegistroPlaga.delete(id);
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-border">
      ...
      <div className="p-4 space-y-3">
        ...
        {visibles.map((r) => {
          const styles = ESTADO_STYLES[r.estado] || ESTADO_STYLES.correcto;
          return (
            <div key={r.id} className={`rounded-xl border px-5 py-4 ${styles.card}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-foreground text-base">
                      {r.tipo_elemento ? `${r.tipo_elemento} - ${r.punto_nombre}` : r.punto_nombre}
                    </span>
                  ...
                  {r.ubicacion && (
                    <p className="text-sm text-muted-foreground mb-1">{r.ubicacion}</p>
                  )}
                ...
                <button onClick={() => handleDelete(r.id)} className="text-destructive/60 hover:text-destructive transition-colors ml-3">
