    ...
    <div className="bg-secondary p-5 space-y-5 border-b border-border/40">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select value={local.zona} onChange={(e) => set("zona", e.target.value)}
            className="w-full h-9 rounded-lg border border-input bg-white px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
...
export default function ListaRegistrosLimpieza({ refreshKey }) {
  ...
  async function fetchPage(skipVal, replace = false) {
    ...
    const data = await base44.entities.RegistroLimpieza.filter({ business_id: currentBusiness.id }, "-fecha", PAGE_SIZE + 1, skipVal);
          ...
          {filtrados.map((r) => {
            ...
            return (
              <div key={r.id} className={`${styles.card} rounded-xl border px-5 py-4 relative`}>
                <button onClick={() => handleDelete(r.id)} className="absolute top-4 right-4 text-destructive/60 hover:text-destructive transition-colors"><Trash2 className="w-4 h-4" /></button>
                ...
                {r.productos_usados?.length > 0 && (
                  <p className="text-sm text-foreground mb-2"><span className="font-semibold">Producto utilizado: </span><span className="text-[#6BB68A]">{r.productos_usados.join(", ")}</span></p>
                )}
                {r.motivo_no_limpiado && <p className="text-sm text-red-500 italic mb-1">Motivo: {r.motivo_no_limpiado}</p>}
                {r.comentario && <p className="text-xs text-muted-foreground italic mb-1">{r.comentario}</p>}
                {r.foto_url && <img src={r.foto_url} alt="foto zona" className="h-20 rounded-lg object-cover border border-border mt-1 mb-2" />}
