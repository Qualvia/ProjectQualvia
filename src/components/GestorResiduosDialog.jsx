...
export default function GestorResiduosDialog({ open, onOpenChange }) {
  ...
  async function handleSave() {
    ...
    if (existingId) {
      await base44.entities.GestorResiduos.update(existingId, payload);
    } else {
      await base44.entities.GestorResiduos.create(payload);
    }
    setSaving(false);
    onOpenChange(false);
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 pt-2">
          <div>
            ...
            <Input placeholder="Ej: Gestión Residuos S.L." value={form.nombre_empresa}
          ...
          <div>
            <Label className="mb-1.5 block">Contrato / Autorización</Label>
            {form.contrato_url ? (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-border bg-secondary">
                ...
                <a href={form.contrato_url} target="_blank" rel="noreferrer" className="text-sm text-[#6BB68A] underline flex-1 truncate">Ver documento subido</a>
