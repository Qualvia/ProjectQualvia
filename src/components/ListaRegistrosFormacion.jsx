...
import { Loader2, Trash2, Filter, CalendarDays, User, ChevronDown, ExternalLink } from "lucide-react";
...
function aplicarFiltros(registros, filtros) {
  return registros.filter((r) => {
    const fecha = r.fecha_formacion ? new Date(r.fecha_formacion) : null;
...
export default function ListaRegistrosFormacion({ refreshKey }) {
          ...
          <div key={r.id} className="bg-white rounded-xl border border-border px-5 py-4">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <p className="font-bold text-foreground text-base mb-1">{r.tema}</p>

                {r.fecha_formacion && (
                  <p className="text-sm text-foreground mb-0.5">
                    <span className="font-semibold">Fecha: </span>
                    {format(new Date(r.fecha_formacion), "d MMM yyyy", { locale: es })}
                  </p>
                )}
                {r.duracion_horas && (
                  <p className="text-sm text-foreground mb-0.5">
                    <span className="font-semibold">Duración: </span>
                    <span className="text-[#6BB68A]">{r.duracion_horas}h</span>
                  </p>
                )}
                {r.formador && (
                  <p className="text-sm text-foreground mb-0.5">
                    <span className="font-semibold">Formador: </span>{r.formador}
                  </p>
                )}
                {r.empleados_asistentes && (
                  <p className="text-sm text-foreground mb-0.5">
                    <span className="font-semibold">Asistentes: </span>{r.empleados_asistentes}
                  </p>
                )}
                {r.contenido && (
                  <p className="text-xs text-muted-foreground italic mt-1 mb-1">{r.contenido}</p>
                )}
                {r.observaciones && (
                  <p className="text-xs text-muted-foreground italic">{r.observaciones}</p>
                )}
              ...
              <div className="flex gap-2 ml-3 shrink-0">
                {r.evidencia_url && (
                  <a href={r.evidencia_url} target="_blank" rel="noreferrer" className="text-[#6BB68A] hover:opacity-70 transition-opacity">
