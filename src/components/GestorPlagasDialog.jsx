...
export default function GestorPlagasDialog({ open, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    base44.entities.GestorPlagas.filter({ business_id: currentBusiness.id })
      .then((data) => {
        if (data.length > 0) {
          setForm(data[0]);
  ...
  async function handleSave() {
    ...
    if (existingId) {
      await base44.entities.GestorPlagas.update(existingId, payload);
    } else {
      await base44.entities.GestorPlagas.create(payload);
    }
    setSaving(false);
    onOpenChange(false);
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 pt-2">
          <div>
            ...
            <Input
              placeholder="Ej: Control Plagas S.L."
